...
def purification(matrix,max_iterations=10,precision=1e-9):
    ...
    while (max_val>precision):
        matrix=3*np.matmul(matrix,matrix)-2*np.matmul(matrix,np.matmul(matrix,matrix))
        counter+=1
        max_val=np.max(np.abs(np.matmul(matrix,matrix)-matrix))
...
print("Trace of almost idempotent matrix %3.3f"%np.trace(P_1))
print("Trace purifie matrix %3.3f"%np.trace(P_1_pure))


#eigenvals,_,eigenvectors,_,_=lapack.dgeev(P_1_pure)
eigenvals,eigenvectors=np.linalg.eig(P_1_pure)


P_1_in_new_base=np.matmul(eigenvectors.conj().T,np.matmul(P_1,eigenvectors))
...
with np.printoptions(precision=3, suppress=True):
    ...
    print(np.matmul(eigenvectors.conj().T,np.matmul(P_1_pure,eigenvectors)))
